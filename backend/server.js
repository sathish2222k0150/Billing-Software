...
app.post("/service-invoice", (req, res) => {
  ...
  db.query(sql, values, (err, result) => {
    ...
    const partSql = `
      ...
      (invoice_id, part_no, hsn_sac, vehicle, part_description, qty, mrp, rate, cgst, sgst) 
      VALUES ?`;
  ...
  db.getConnection((err, connection) => {
    ...
    connection.query(
      `SELECT 
        ...
        (SELECT COUNT(*) FROM estimates) AS estimateInvoiceCount,
        (SELECT COALESCE(SUM(total_amount), 0) FROM estimates WHERE document_type='estimate') AS estimateInvoiceRevenue,
