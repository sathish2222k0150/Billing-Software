...
function Estimate() {
  ...
  const [customerDetails, setCustomerDetails] = useState({
    ...
    model: "",
    regNo: "",
    estimateDate: formatDate(new Date().toISOString().split("T")[0]),
  });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status === 200) {
        alert(`${documentType === 'estimate' ? 'Estimate' : 'Invoice'} saved successfully!`);
        navigate("/EstimatePreview", {
          state: {
            selectedParts: partsWithDefaults,
            customerDetails,
            estimateId: response.data.estimateId,
            documentType  // Pass document type to preview
          },
        });
      }
    } catch (error) {
      console.error(`Error saving ${documentType}:`, error);
      alert(`Failed to save ${documentType}. Try again!`);
    }
  };
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="col-span-2 flex items-center space-x-4">
          <label className="inline-flex items-center">
            <input
              ...
              onChange={() => setDocumentType('estimate')}
            />
      ...
      <div className="grid grid-cols-2 gap-4 bg-white p-6 rounded shadow-md mb-6">
        ...
        <input
          type="date"
          name="estimateDate"
          value={customerDetails.estimateDate.split('-').reverse().join('-')} // Convert back to yyyy-mm-dd for the input
          onChange={(e) => {
            const formattedDate = formatDate(e.target.value);
            setCustomerDetails({...customerDetails, estimateDate: formattedDate});
          }}
          className="border p-2 w-full"
          required
        />
