...
function LabourInvoice() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post("http://localhost:5000/save-labour-invoice", {
        customerDetails,
        labourItems: labourItems.map(item => ({
          ...item,
          tinkering: parseFloat(item.tinkering) || 0,
          painting: parseFloat(item.painting) || 0,
          electrician: parseFloat(item.electrician) || 0,
          mechanical: parseFloat(item.mechanical) || 0,
            ...
            <tr className="bg-gray-100">
              ...
              <td className="p-2">
                <button
                  ...
                  className="flex items-center justify-center w-full bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600"
                  title="Add new item"
                >
